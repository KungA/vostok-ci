...
async function build(): Promise<void> {
  ...
  const cementArchive = await tc.downloadTool(
    'https://github.com/skbkontur/cement/releases/download/v1.0.71/eed45d0e872e6d783b3a4eb8db0904f574de7018.zip'
  )
  ...
  const projectFilesGlobber = await glob.create(
    [`${moduleFolder}/*/*.csproj`, `!${moduleFolder}/*.Tests/*.csproj`].join(
      '\n'
    )
  )
  ...
  const testFilesGlobber = await glob.create(
    [`${moduleFolder}/*.Tests/*.csproj`].join('\n')
  )
  ...
  if (isMaster && !isRelease) {
    ...
    await execTool(
      'dotnetversionsuffix',
      [`pre${String(github.context.runNumber).padStart(6, '0')}`],
      {cwd: moduleFolder}
    )
  }
...
async function test(): Promise<void> {
  ...
  await exec.exec(
    ...
    ],
    {
      ...
      listeners: {
        stdline: line => {
          if (line.includes('Total:   ')) core.notice(line)
        }
      }
    }
  )
...
async function publish(): Promise<void> {
  ...
  await exec.exec('dotnet', ['pack', '-c', 'Release', '--no-build'], {
    cwd: moduleFolder
  })

  core.startGroup('Publish')
  const packagesGlobber = await glob.create(
    [`${moduleFolder}/**/*.nupkg`, `${moduleFolder}/**/*.snupkg`].join('\n')
  )
